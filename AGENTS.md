...
#### 1. 永続的ドキュメント（`docs/development/permanent/`）
...
#### 2. 作業統括ドキュメント（`docs/development/.steering/steering.md`）

`docs/development/.steering/steering.md` は、複数ターミナルによる並列開発を前提にした「作業キュー/着手順/進捗の可視化」のための統括ドキュメントです。

目的:

- 作業単位（`docs/development/.steering/<作業ID>-.../`）の一覧化と、進捗状況の更新による可視化
...
#### 3. 作業単位のドキュメント（`docs/development/.steering/[作業ID]-[YYYYMMDD]-[開発タイトル]/`）
...
`docs/development/.steering/` 配下は以下を基本構造とします。

- `docs/development/.steering/steering.md`（作業統括ドキュメント）
- `docs/development/.steering/[作業ID]-[YYYYMMDD]-[開発タイトル]/`（作業単位ディレクトリ）

### ステアリングディレクトリの命名規則

```bash
docs/development/.steering/[作業ID]-[YYYYMMDD]-[開発タイトル]/
```
...
- 依存関係の一次ソースは `docs/development/.steering/steering.md` に明示する（番号だけで全依存を表現しない）

注意（並び順）:

- OS やツールの表示順（文字列ソート）と、`N.M` の数値的な順序は一致しない場合がある。
- 表示順は `docs/development/.steering/steering.md` を一次ソースとし、着手順はそこに明記する。
- 並び順の安定性を上げたい場合は、将来的に `01.00`, `01.01` のようなゼロ埋めを採用してもよい（採用する場合はプロジェクト内で統一する）。

例:

- 直列（単独）:
  - `docs/development/.steering/1.0-20250103-initial-implementation/`
  - `docs/development/.steering/2.0-20250115-add-tag-feature/`
  - `docs/development/.steering/3.0-20250120-fix-filter-bug/`

- 並列（同一フェーズ内で分割）:
  - `docs/development/.steering/1.1-20250103-add-task-create-ui/`
  - `docs/development/.steering/1.2-20250103-add-task-list-ui/`
  - `docs/development/.steering/1.3-20250103-add-task-edit-ui/`
  - `docs/development/.steering/2.1-20250115-add-tag-schema/`
  - `docs/development/.steering/2.2-20250115-add-tag-ui/`
...
- 「不要」と判断した場合:
  ...
  - PR 前の `document-update` でも `docs/development/.steering/steering.md` を更新/作成しない。

#### 1. 影響分析

- 永続的ドキュメント（`docs/development/permanent/`）への影響を確認
- 変更が基本設計に影響する場合は `docs/development/permanent/` を更新

#### 2. 作業統括ドキュメント更新（`docs/development/.steering/steering.md`）

- `docs/development/.steering/steering.md` が無ければ作成する。
...
- 1 ブランチで複数の作業ID（複数の `docs/development/.steering/<作業ID>-.../`）を同時に進めない。
...
```bash
mkdir -p docs/development/.steering/[作業ID]-[YYYYMMDD]-[開発タイトル]
```

例:

```bash
mkdir -p docs/development/.steering/1.0-20250115-add-tag-feature
```
...
変更が基本設計に影響する場合、該当する `docs/development/permanent/` 内のドキュメントを更新します。

#### 7. 実装開始

- `docs/development/.steering/[作業ID]-[YYYYMMDD]-[開発タイトル]/tasklist.md` に基づいて実装を進めます。
- `docs/development/.steering/steering.md` の状態を `doing` に更新します（必要な場合）。
- 作業中の進捗更新は以下を一次とする:
  - 詳細: `tasklist.md`
  - 俯瞰/状態: `steering.md`
- ※ ステアリング不要と判断した場合は本ステップを実施しない。
...
### 永続的ドキュメント（`docs/development/permanent/`）
...
### 作業統括ドキュメント（`docs/development/.steering/steering.md`）
...
### 作業単位のドキュメント（`docs/development/.steering/`）
...
- 原則として「1 作業単位（`docs/development/.steering/<作業ID>-.../`）= 1 ブランチ = 1 PR」で運用する。
...
- 開発途中の save point（能動的に実行）:
  - `$checkpoint-save`（`$verify-fast` を通したうえで checkpoint commit/push を行う。作業完了時は使わず `$pr-flow` に進む）
  - 以下のいずれかに該当したら能動的に実行する:
    - 1 つの論理単位（関数・コンポーネント・ファイル群）の実装が完了したとき
    - 次の作業単位に移る前
    - 差分が肥大化する前（目安: 複数ファイルにまたがる変更が溜まったとき）
...
- `docs/development/permanent/repository-structure.md` は `bash scripts/tree.sh` により生成/更新する（手動編集しない）。
