...
- 統合先の決定までは read-only の調査を優先し、早い段階で rename/delete に入らない。
...
- 候補が複数あり、ownership や責務境界が曖昧なら `code-mapper` を使う。
- 統合作業が rename や構造整理を伴う場合のみ `refactoring-specialist` を使う。
- この skill の段階では、まず「どこへ寄せるか」の判断を固める。
...
- 統合先が決まる前に、いきなり既存実装の削除や rename をしない。
